...
static void test_valid_nrom_16kb(void) {
  ...
  TEST("mirroring = horizontal", cart.mapper.mirroring == SB_MIRROR_HORIZONTAL);
  TEST("mapper = 0", cart.mapper_id == 0);
  TEST("prg_mirror = true (only 16KB)", cart.mapper.PRG_banks <= 1);
}
...
static void test_valid_nrom_32kb(void) {
  ...
  TEST("mirroring = vertical", cart.mapper.mirroring == SB_MIRROR_VERTICAL);
  TEST("prg_mirror = false (32KB fills space)", cart.mapper.PRG_banks > 1);
}
...
static void test_nes2_rejected(void) {
  ...
  sb_cartridge_result_t r = sb_cartridge_load(&cart, rom, 16 + 16384);
  free(rom);
  TEST("NES 2.0 format returns ERR_NES20", r == SB_CARTRIDGE_ERR_NES20);
}
...
static void test_sram(void) {
  ...
  cart.mapper.write(&cart.mapper, 0x6000, 0xAB);
  uint8_t v = cart.mapper.read(&cart.mapper, 0x6000);
  TEST("SRAM write+read at $6000", v == 0xAB);

  v = cart.mapper.read(&cart.mapper, 0x6001);
  TEST("SRAM $6001 = 0 (not written)", v == 0x00);

  // Write to PRG area ($8000) should be ignored
  cart.mapper.write(&cart.mapper, 0x8000, 0xFF);
  v = cart.mapper.read(&cart.mapper, 0x8000);
...
static void test_chr_read(void) {
  ...
  uint8_t v = cart.mapper.read_chr(&cart.mapper, 0x0000);
  TEST("CHR-ROM read at $0000", v == 0xFF);

  cart.mapper.write_chr(&cart.mapper, 0x0000, 0x00);
  v = cart.mapper.read_chr(&cart.mapper, 0x0000);
...
static void test_chr_ram(void) {
  ...
  cart.mapper.write_chr(&cart.mapper, 0x0123, 0xAB);
  v = cart.mapper.read_chr(&cart.mapper, 0x0123);
