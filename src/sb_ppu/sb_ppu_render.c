...
void sb_ppu_render_pixel(sb_ppu_t* ppu) {
  ...
  if (!bg_enabled && !spr_enabled) {
    ...
    if (addr >= 0x3F00)
      ppu->framebuffer[fb_index] = ppu->palette[ppu_mirror_palette_idx(addr & 0x1F)];
    else
  ...
  if (bg_enabled || spr_enabled) {
    if (pixel_x == 0 || ppu->fine_x_counter == 0) {
      ...
      uint16_t pattern_addr = pt_base + ppu->tile_cache.tile_index * 16 + fine_y;
  ...
  if (bg_enabled) {
    if (pixel_x >= 8 || show_bg_left) {
      ...
      bg_color_idx = ((ppu->tile_cache.high >> bit) & 1) << 1 | ((ppu->tile_cache.low >> bit) & 1);
