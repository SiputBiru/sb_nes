...
// NES button bitmask (same as sb_frontend.c)
...
// ---- Shared init (same defaults as sb_frontend.c) ----
...
// ---- Controller input via GetAsyncKeyState ----

static uint8_t read_controller(void) {
  uint8_t btns = 0;
  if (GetAsyncKeyState('W') & 0x8000) btns |= BTN_UP;
  if (GetAsyncKeyState('A') & 0x8000) btns |= BTN_LEFT;
  if (GetAsyncKeyState('S') & 0x8000) btns |= BTN_DOWN;
  if (GetAsyncKeyState('D') & 0x8000) btns |= BTN_RIGHT;
  if (GetAsyncKeyState('J') & 0x8000) btns |= BTN_B;
  if (GetAsyncKeyState('K') & 0x8000) btns |= BTN_A;
  if (GetAsyncKeyState(VK_SPACE) & 0x8000) btns |= BTN_SELECT;
  if (GetAsyncKeyState(VK_RETURN) & 0x8000) btns |= BTN_START;
  return btns;
}

// ---- Render NES framebuffer to window via GDI ----
...
static void render_frame(HWND hwnd, sb_nes_t* nes) {
  ...
  HDC hdc = GetDC(hwnd);
  StretchDIBits(hdc,
    0, 0, win_w, win_h,            // dest rect (scaled to window)
    0, 0, 256, 240,                // src rect (full framebuffer)
...
void sb_frontend_run(sb_nes_t* nes, sb_frontend_config_t* config) {
  ...
  HWND hwnd = CreateWindowEx(
    0, CLASS_NAME, "sb_nes — SiputBiru NES Emulator",
